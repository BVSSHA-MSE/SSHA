...
## Scope & Boundaries
- Apply only to BVSSH outcomes (`bvssh/YYYY/<level>/<theme>-<outcome-slug>.md`) and related review notes.
...
- Execute all steps in order: Year/Level + Theme → Name → Context → BVSSH dimensions → Hypothesis → Behavioral change → Signals → Constraints → Review cadence → Reflection (post-outcome when applicable).
...
- Suggested path: `bvssh/YYYY/<level>/<theme>-<outcome-slug>.md` (lowercase hyphenated slug from the outcome name).
